...
int main(int argc, char* argv[]) {
    ...
    // Brief delay for parent to finish PCB initialization
    usleep(1000);  // Just 1ms
    
    // Sanity check - if clock seems invalid, wait a bit more
    ...
    // Calculate next action time (random 0-100ms from current clock)
    ...
    int loopCount = 0;
    while (1) {
        // Force termination check every 100 iterations even if time hasn't passed
        loopCount++;
        bool forceTermCheck = (loopCount % 100 == 0);
        
        // Check if time to terminate
        if (forceTermCheck || timePassed(sharedClock, terminateSeconds, terminateNanoSeconds)) {
        ...
        if (timePassed(sharedClock, nextActionSeconds, nextActionNanoSeconds)) {
            ...
            if (shouldRequest) {
                ...
                // Make sure we don't exceed maximum instances
                ...
                if (amountToRequest > 0) {
                    // Check if this violates resource ordering
                    ...
                    for (int i = 0; i < resourceToRequest; i++) {
                        if (myResources[i] > 0) {
                            // We have a lower numbered resource
                            // Check if we have any higher numbered ones
                    ...
                    if (needsMassRelease) {
                        ...
                        // Update my resource counts
                        for (int i = 0; i < NUM_RESOURCES; i++) {
                            myResources[i] -= releaseResources[i];
                        }
                        
                        // Now request everything back plus the new resource
                        ...
                        // Wait for grant (might be blocked)
                        Message grant;
                        pid_t myPid = getpid();
